...
import 'package:luminar_std/core/utils/phone_utils.dart';
                            ...
                            : () async {
                                ...
                                try {
                                  ...
                                  if (context.mounted) {
                                    ...
                                    // Show the exact backend validation message. `e` is an
                                    // Exception(result.message) from submitProfile, and
                                    // e.toString() prefixes "Exception: " — strip that rather
                                    // than routing through AppUtils.friendlyError, which
                                    // unconditionally replaces any string containing the word
                                    // "exception" with a generic message, hiding the real
                                    // validation reason (e.g. field-specific errors).
                                    final rawMessage = e
                                        .toString()
                                        .replaceFirst('Exception: ', '');
                                    _showErrorDialog(rawMessage);
                                  }
                                }
                              },
                        style: ElevatedButton.styleFrom(
...
class PersonalInfoSectionState extends State<PersonalInfoSection>
  ...
  void _initData(ProfileController controller) {
    ...
    if (pInfo != null) {
      ...
      if (_hasPhone) {
        PhoneUtils.splitPhone(pInfo.phone!, (code, digits) {
          _countryCode = code;
        ...
        PhoneUtils.splitPhone(pInfo.whatsappNumber!, (code, digits) {
          _countryCode = code;
          _phoneController.text = digits;
        });
                            ...
                            Container(
                              ...
                              child: CountryCodePicker(
                                ...
                                onChanged: (code) {
                                  ...
                                  // Keep the text field digits-only — the dial code is
                                  // already shown by the picker itself.
                                  final digits = _phoneController.text
                                      .replaceFirst(RegExp(r'^\+\d+\s*'), '');
                                  context.read<CompleteProfileController>().phone =
                                      '$dial $digits';
                                },
                                initialSelection: _countryCode ?? 'IN',
                                favorite: ['+91', '+1', '+44'],
...
class ParentInfoSectionState extends State<ParentInfoSection>
  ...
  void _initData(ProfileController controller) {
    ...
    if (cInfo != null) {
      ...
      if (_hasParentPhone) {
        PhoneUtils.splitPhone(cInfo.parentPhone!, (code, digits) {
          _parentCountryCode = code;
          _phoneController.text = digits;
        });
                            ...
                            Container(
                              ...
                              child: CountryCodePicker(
                                ...
                                onChanged: (code) {
                                  ...
                                  context
                                      .read<CompleteProfileController>()
                                      .parentPhone = '$dial $digits';
                                },
                                initialSelection: _parentCountryCode,
                                favorite: ['+91', '+1', '+44'],
                                showCountryOnly: false,
                                showOnlyCountryWhenClosed: false,
                                alignLeft: false,
